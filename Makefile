...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "fishfarm", "tests")); system.time(test_check("fishfarm", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran fishfarm_$(VERSION).tar.gz
	rm -rf fishfarm.Rcheck/
